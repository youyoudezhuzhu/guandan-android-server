...
> **当前版本 v1.6.4**：新增 10 种技能模式、技能状态头像标记、技能日志规范、声东击西/弃牌弹窗优化。
...
- 🎖️ **技能模式（单机/联机可选）**：10 种技能互搏——无中生有（抽两张）/ 顺手牵羊（偷一张）/ 过河拆桥（弃目标一张）/ 乐不思蜀（跳过下回合）/ 五谷登丰（未出完者各+1张）/ 移花接木（与目标换牌）/ 明察秋毫（偷看目标手牌）/ 偷梁换柱（弃一抽一）/ 空城计（本回合免疫目标技能）/ 声东击西（锁定目标技能，需耗牌解除），开局每人发 2 张技能卡，AI 也会用技能
- 🏷️ **技能状态头像标记**：声东击西🧭 / 乐不思蜀💤 / 空城计🛡 直接在对应玩家头像上打角标，一目了然

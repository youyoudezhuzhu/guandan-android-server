(() => {
 ...
 tutorial7Title: "Level up together", tutorial7Copy: "The first finisher and partner rank decide the level gain. Win while playing A to finish the match.",
  ...
  const renderTutorial = () => {
    ...
    $("tutorial-count").textContent = `${tutorialStep + 1} / ${total}`;
    $("tutorial-prev").disabled = tutorialStep === 0;
    $("tutorial-next").textContent = tutorialStep === total - 1 ? t("finish") : t("next");
  };
  ...
  $("tutorial-next").addEventListener("click", () => {
    ...
    if (tutorialStep === total - 1) { leaveRoom(); close($("tutorial-dialog")); showGame("singlePlayer"); window.GuandanGame?.startSingle(); }
